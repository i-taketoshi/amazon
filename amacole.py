...
def getASIN(localsoup):
    atags= localsoup.findAll('a')
    #ASINを抜き出す正規表現
    #否定先読みで数字のみのものと%%で囲まれているものは除外
    #r = re.compile(r'(?!%[A-Z0-9]{10}%)[^a-zA-Z0-9](?![0-9]{10})([A-Z0-9]{10})[^a-zA-Z0-9]')
    r = re.compile(r'/(?![0-9]{10})([A-Z0-9]{10})/')
    
    asins = []
    for a in atags:
        try:
            
            decodehref = urllib.parse.unquote(a.get('href'))
            m = r.search(decodehref)
            if m:
                #print(m.group(1))
    ...
    asins_uniq = list(set(asins))
    with open(asin_path, "a") as file:
        for x in asins_uniq:
            file.write(str(x) + "\n")
...
#除外リスト読み込み
asin_path = r"D:\workspace\アマゾン出品大学\Office_asinlist.txt"
exclusion_path = r"D:\workspace\アマゾン出品大学\Office_exclusion_list.txt"
f = open(exclusion_path, 'r')
exclusion_list =  f.read().split('\n')
f.close()
file_path = r"D:\workspace\アマゾン出品大学\Office_Recommended_Item_List.xlsx"
...
ws.cell(row = column_name_row, column = 15).value = "販売者"
ws.cell(row = column_name_row, column = 16).value = "発送者"
ws.cell(row = column_name_row, column = 17).value = "判定"
ws.cell(row = column_name_row, column = 18).value = "評価"
ws.cell(row = column_name_row, column = 19).value = "評価数"
ws.cell(row = column_name_row, column = 20).value = "順位"
ws.cell(row = column_name_row, column = 21).value = "状態"
ws.cell(row = column_name_row, column = 22).value = "販売者"
ws.cell(row = column_name_row, column = 23).value = "送料"
